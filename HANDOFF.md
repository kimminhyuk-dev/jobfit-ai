...
- 사용자 이력서 화면(`/user/resumes`) 실제 API 연결 완료
  ...
  - 사용자 화면에는 파싱 데이터 탭을 노출하지 않고 파일 미리보기만 표시
...
- 사용자 이력서 화면(`/user/resumes`)
  - 파싱 데이터 탭과 파싱 JSON 수정 UI 제거
  - 파일 업로드, 목록, 상태, 원본 파일 미리보기 중심으로 정리
  - 사용자용 `PATCH /resumes/{resume_id}` 파싱 수정 API 제거
- 관리자 파싱 화면
  - 프로젝트 카드가 `parsed_data.projects`뿐 아니라 `parsed_data.sections.projects`와 자기소개서 목차 중 `기술역량 및 프로젝트/프로젝트 내용/프로젝트 후기` 계열도 함께 표시하도록 보강
  - 기존 저장 데이터에서 프로젝트 내용이 다른 섹션에 들어간 경우에도 관리자 화면에서 누락되지 않도록 처리
- 검증:
