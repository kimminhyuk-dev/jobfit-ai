...
from app.services.job_sources.alio_client import (
    AlioApiKeyMissingError,
    AlioClient,
    AlioClientError,
    AlioJsonParseError,
)
...
class AlioCollectionError(Exception):
    """ALIO 수집 실패"""


class AlioCollectionService:
    """ALIO 채용정보 수집을 조율한다."""
    ...
    def collect_jobs(
        ...
        """ALIO 목록 API에서 채용공고를 page 단위로 수집한다."""
        try:
            self.job_source_service.ensure_collectable("ALIO")
        except JobSourceUnavailableError as exc:
            return self._record_blocked_run(
                params=params,
        ...
        except AlioApiKeyMissingError:
            final_status = "FAILED"
            error = {
                "error_code": "JOB_005",
                "error_message": "ALIO API 키가 설정되지 않았습니다.",
            }
        except AlioJsonParseError:
            final_status = "FAILED"
            error = {
                "error_code": "JOB_004",
                "error_message": "ALIO JSON 응답 파싱에 실패했습니다.",
            }
        except AlioClientError:
            final_status = "FAILED"
            error = {
                "error_code": "JOB_003",
                "error_message": "ALIO 외부 API 호출에 실패했습니다.",
            }
        except Exception as exc:
            final_status = "FAILED"
            error = {
                "error_code": "BATCH_001",
                "error_message": "ALIO 채용정보 수집 배치가 실패했습니다.",
            }
            self.db.rollback()
            raise AlioCollectionError("ALIO 채용정보 수집에 실패했습니다.") from exc
        else:
    ...
    def _record_blocked_run(
        ...
        """ALIO 수집원이 ACTIVE가 아닐 때 외부 호출 없이 차단 이력을 남긴다."""
        blocked_key = make_blocked_idempotency_key("ALIO", idempotency_key)
        if blocked_key:
        ...
        run = self.batch_job_run_repository.create_run(
            ...
            request_params={**params, "source_status": source_status},
        )
        self.db.commit()
        self.db.refresh(run)
    ...
    def _collect_pages(
        ...
        for page_offset in range(max_pages):
            if page_offset > 0:
                self.alio_client.wait_between_requests()

            recruits = self.alio_client.fetch_recruit_list(
                keyword=params.get("keyword"),
                start_page=start_page + page_offset,
                display=display,
            )
            counts["collected_count"] += len(recruits)

            for recruit in recruits:
                try:
                    _, save_status = self.job_posting_service.save_from_alio(
                        run=run,
                        alio_data=recruit,
                    )
